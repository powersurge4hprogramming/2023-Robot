...
public class MathU {
  ...
  /**
   * Maps from 0 to 1 onto a to b, preserving the sign of tau
   * tau is value from zero to one
   * if a = 1, b = 6, tau = 0.5, then result = 3.5
   */
  public static final double signLerp(double a, double b, double tau) {
    double multiplier = Math.signum(tau);
    tau = Math.max(Math.abs(tau), 0.0);
    tau = Math.min(Math.abs(tau), 1.0);
    return multiplier * (b * tau) + (a * (1 - tau));
  }
